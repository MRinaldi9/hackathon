...
//  Stack: Fastify. Stato in memoria (basta per la demo).
...
const SIGMA0     = 1.0;   // incertezza iniziale (alta)
...
function aggiorna(stato, concetto, difficolta, esatto) {
  ...
  a.sigma  = Math.max(SIGMA_MIN * 0.6, a.sigma * 0.8); // l'incertezza si stringe
  a.risposte += 1;
}
...
// Costruisce il profilo finale: forte / debole per concetto
...
app.listen({ port: 3000 }, (err) => {
  ...
  console.log('Motore di stima in ascolto su http://localhost:3000');
});
